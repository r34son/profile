...
resource "yandex_iam_service_account" "service_account" {
  name        = "${local.prefix}${var.function_name}-sa"
  description = "Service account for invoking server function"
}
...
resource "yandex_function" "server_function" {
  name = "${local.prefix}${var.function_name}${local.suffix}"
  #   TODO:
  user_hash  = local.should_use_zip ? try(var.function_code.zip.hash) : uuid()
  entrypoint = var.handler
  runtime    = var.runtime
  memory     = var.memory_size
  #   content {
  #     zip_filename = local.should_use_zip ? try(var.function_code.zip.path) : null
  #   }
  package {
    bucket_name = local.should_use_zip ? null : var.function_code.s3.bucket
    object_name = local.should_use_zip ? null : var.function_code.s3.key
  }
  execution_timeout  = var.timeout
  service_account_id = yandex_iam_service_account.service_account.id

  environment = var.environment_variables


  # TODO: support s3 bucket source code
